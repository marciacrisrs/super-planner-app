...
class PlanImportInterpreter @Inject constructor(
    ...
    suspend fun interpret(uri: Uri): Proposal {
        val name = uri.toString().substringAfterLast('/').ifBlank { "Documento importado" }
        val mime = context.contentResolver.getType(uri).orEmpty()
        val bytes = context.contentResolver.openInputStream(uri)?.use { it.readBytes() } ?: return Proposal(name, emptyList(), "Não foi possível ler o arquivo.")
        val text = when {
        ...
        val lines = text.lines().map { it.replace(Regex("\\s+"), " ").trim() }
            .filter { it.length >= 3 }
            .distinct().take(30)
        val items = lines.map { line ->
            val duration = Regex("(\\d{1,3})\\s*(min|mins|minutos)", RegexOption.IGNORE_CASE).find(line)?.groupValues?.get(1)?.toIntOrNull() ?: 30
            ProposalItem(line.take(120), duration, duration == 30)
        }
        return Proposal(name, items, if (text.isBlank()) "Não foi possível extrair texto automaticamente; revise o documento manualmente." else null)
    }
