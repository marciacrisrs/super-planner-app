...
class RoomPlanRepository @Inject constructor(private val dao: PlanDao) : PlanRepository {
    ...
    fun observeItems(planId: String): Flow<List<PlanItem>> =
        dao.observeItems(planId).map { it.map(::itemToDomain) }
    ...
    private fun toEntity(plan: Plan) = PlanEntity(
        ...
        plan.origin,
        plan.status.name,
        plan.validFrom?.toString(),
        plan.validUntil?.toString(),
        plan.sourceDocument,
        plan.version,
    )
    ...
    private fun toDomain(row: PlanEntity) = Plan(
        ...
        row.origin,
        runCatching { PlanStatus.valueOf(row.status) }.getOrDefault(PlanStatus.ACTIVE),
        row.validFrom?.let(LocalDate::parse),
        row.validUntil?.let(LocalDate::parse),
        row.sourceDocument,
        row.version,
    )
    ...
    private fun itemToEntity(item: PlanItem) = PlanItemEntity(
        ...
        item.daysOfWeek.joinToString(",") { it.name },
        item.time?.toString(),
        null,
        item.notes,
    )
