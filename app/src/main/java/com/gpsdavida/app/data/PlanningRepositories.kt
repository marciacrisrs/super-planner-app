...
import com.superplanner.app.data.local.GoalDao
import com.superplanner.app.data.local.GoalEntity
import com.superplanner.app.data.local.InboxItemDao
import com.superplanner.app.data.local.InboxItemEntity
import com.superplanner.app.data.local.MilestoneDao
import com.superplanner.app.data.local.MilestoneEntity
import com.superplanner.app.data.local.ProjectDao
import com.superplanner.app.data.local.ProjectEntity
...
class RoomGoalRepository @Inject constructor(private val dao: GoalDao) : GoalRepository {
    override fun observeAll(): Flow<List<Goal>> =
        dao.observeAll().map { rows -> rows.map { row -> Goal(GoalId(row.id), row.title) } }

    override suspend fun getById(id: GoalId): Goal? =
        dao.getById(id.value)?.let { Goal(GoalId(it.id), it.title) }
...
class RoomProjectRepository @Inject constructor(private val dao: ProjectDao) : ProjectRepository {
        ...
        project.steps
            ...
            .joinToString("\n") { step ->
                listOf(
                    ...
                    step.order,
                    step.completed,
                    step.plannedDuration.toMinutes(),
                    step.title.replace("\t", " ").replace("\n", " "),
                ).joinToString("\t")
            },
        project.someday,
        project.waitingFor,
    ...
    private fun toDomain(row: ProjectEntity) = Project(
        ProjectId(row.id),
        row.title,
        row.goalId?.let(::GoalId),
        row.steps.lines()
            ...
            .mapNotNull { line ->
                ...
                ProjectStep(
                    ProjectStepId(parts[0]),
                    parts[4],
                    Duration.ofMinutes(parts[3].toLongOrNull() ?: DEFAULT_STEP_DURATION_MINUTES),
                    parts[1].toIntOrNull() ?: DEFAULT_STEP_ORDER,
                    parts[2].toBoolean(),
                )
            },
        row.someday,
    ...
    private companion object {
        const val DEFAULT_STEP_DURATION_MINUTES = 30L
        const val DEFAULT_STEP_ORDER = 0
    }
}
...
class RoomInboxRepository @Inject constructor(private val dao: InboxItemDao) : InboxRepository {
    ...
    override suspend fun getById(id: InboxItemId): InboxItem? =
        dao.getById(id.value)?.let(::toDomain)
    ...
    private fun toEntity(item: InboxItem) = InboxItemEntity(
        item.id.value,
        item.text,
        item.status.name,
        item.goalId?.value,
    ...
    private fun toDomain(row: InboxItemEntity) = InboxItem(
        InboxItemId(row.id),
        row.text,
        runCatching { InboxStatus.valueOf(row.status) }.getOrDefault(InboxStatus.INBOX),
        row.goalId?.let(::GoalId),
        ...
        dao.observeAll().map { rows ->
            rows.map { row ->
                Milestone(
                    row.id,
                    row.title,
                    LocalDate.ofEpochDay(row.targetEpochDay),
                    row.goalId?.let(::GoalId),
                )
            }
        }

    override suspend fun save(milestone: Milestone) =
        dao.upsert(
            MilestoneEntity(
                milestone.id,
                milestone.title,
                milestone.targetDate.toEpochDay(),
