...
class NextActionGatewayClient @Inject constructor() {
    suspend fun recommend(
        request: AiRequest,
        eligibleCandidateIds: List<String>,
    ): RemoteNextActionRecommendation = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            require(responseBody.isNotBlank()) {
                "AI gateway returned an empty next-action response [requestId=$requestId]"
            }

            val root = JSONObject(responseBody)
            val result = root.getJSONObject("result")
            val candidateId = result.optString("recommendedAction")
            ...
            require(alternatives.size <= 2) {
                "Gateway returned too many alternatives [requestId=$requestId]"
            }
