...
/** Converts confirmed Plan items into ordinary Programação inputs.
 *  The Plan itself remains unchanged; the generated tasks/events are what the
 *  adaptive Super Planner can reschedule around availability and conflicts.
 */
class GeneratePlanProgramming @Inject constructor(
    ...
    suspend operator fun invoke(plan: Plan, zone: ZoneId = ZoneId.systemDefault()) {
        val items = plans.observeItems(plan.id).firstSnapshot()
        items.forEach { item ->
...
private suspend fun <T> kotlinx.coroutines.flow.Flow<T>.firstSnapshot(): T = kotlinx.coroutines.flow.first(this)
