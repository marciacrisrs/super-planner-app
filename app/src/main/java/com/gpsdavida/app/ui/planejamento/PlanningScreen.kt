...
fun PlanningScreen(
    ...
    Scaffold { padding ->
        LazyColumn(
            ...
            item {
                Card(Modifier.fillMaxWidth().clickable(onClick = onOpenReview), shape = RoundedCornerShape(24.dp), colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.primaryContainer)) {
                    Row(Modifier.fillMaxWidth().padding(20.dp), verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(14.dp)) {
                        Icon(Icons.Outlined.Refresh, null, Modifier.size(28.dp))
                        Column(Modifier.weight(1f), verticalArrangement = Arrangement.spacedBy(3.dp)) { Text("Revisar minha vida", style = MaterialTheme.typography.titleMedium); Text("Pare, olhe e ajuste o que mudou.", style = MaterialTheme.typography.bodyMedium) }
