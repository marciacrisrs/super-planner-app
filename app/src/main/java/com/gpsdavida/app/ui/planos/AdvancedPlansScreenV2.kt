...
fun AdvancedPlansScreenV2(
    ...
    val openDocument = rememberLauncherForActivityResult(ActivityResultContracts.OpenDocument()) { uri ->
        uri?.let(importViewModel::interpret)
    }
    ...
    if (addItem) {
        ...
        AlertDialog(onDismissRequest = { addItem = false }, title = { Text("Novo item do plano") }, text = { Column(verticalArrangement = Arrangement.spacedBy(8.dp)) { OutlinedTextField(title, { title = it }, label = { Text("Atividade") }); OutlinedTextField(minutes, { minutes = it.filter(Char::isDigit) }, label = { Text("Minutos") }) } }, confirmButton = { Button({ state.selectedId?.let { plansViewModel.addItem(it, title, minutes.toIntOrNull() ?: 30, null, emptySet()) }; addItem = false }) { Text("Salvar") } }, dismissButton = { TextButton({ addItem = false }) { Text("Cancelar") } })
    }
    ...
    importState.proposal?.let { proposal ->
        AlertDialog(
            ...
            confirmButton = {
                Button(onClick = { state.selectedId?.let(importViewModel::confirm) }) { Text("Confirmar") }
            },
            dismissButton = { TextButton(onClick = { }) { Text("Revisar depois") } },
        )
    }
}
