...
fun AdvancedPlansScreenV3(
    ...
    val openDocument = rememberLauncherForActivityResult(ActivityResultContracts.OpenDocument()) { uri ->
        ...
        val target = state.plans.firstOrNull { it.id == importTargetId }
        if (target != null) plansViewModel.attachDocument(target, uri)
                ...
                Card(Modifier.fillMaxWidth()) {
                    Column(Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
                        ...
                        Row(horizontalArrangement = Arrangement.spacedBy(4.dp)) {
                            ...
                            TextButton({ importTargetId = plan.id; plansViewModel.select(plan.id); openDocument.launch(arrayOf("application/pdf", "image/*", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "text/plain")) }) { Text("Importar") }
                            TextButton({ addItem = true; plansViewModel.select(plan.id) }) { Text("Item") }
    ...
    if (addItem) {
        ...
        AlertDialog(
            ...
            confirmButton = { Button(onClick = { state.selectedId?.let { plansViewModel.addItem(it, title, minutes.toIntOrNull() ?: 30, null, emptySet()) }; addItem = false }) { Text("Salvar") } },
    ...
    importState.proposal?.let { proposal ->
        AlertDialog(
            ...
            text = { Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                ...
                proposal.items.take(10).forEach { item -> Text("• ${item.title} · ${item.durationMinutes} min${if (item.uncertain) " · verificar" else ""}") }
            } },
            confirmButton = { Button(onClick = { state.selectedId?.let(importViewModel::confirm) }) { Text("Confirmar") } },
            dismissButton = { TextButton(onClick = { importViewModel.cancel() }) { Text("Cancelar") } },
        )
    }
}
